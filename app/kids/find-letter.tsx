...
// On montre une lettre cible en haut, l'enfant clique la bonne
// parmi 4 choix. 10 questions par partie.
// ============================================================
...
function generateQuestions(): Question[] {
  ...
  // On prend 10 lettres au hasard dans l'alphabet
  const targets = shuffle(ALPHABET).slice(0, TOTAL_QUESTIONS);
  for (const target of targets) {
    // 3 distracteurs au hasard (pas égaux à la cible)
...
export default function FindLetterScreen() {
  ...
  // Quand on change de question, on prononce la lettre cible
  useEffect(() => {
    if (current && !finished) {
  ...
  const onPick = (letter: string) => {
    ...
    // Passage auto à la question suivante après 1.5s
    setTimeout(() => {
  ...
  const replayTarget = () => {
    if (current) void speakNatural(current.target);
  };
          ...
          {finished ? (
            ...
            <>
              ...
              <Text style={styles.instruction}>
                Trouve la lettre{" "}
                <Text style={styles.instructionTarget}>{current.target}</Text>
              </Text>

              {/* Bouton pour réécouter */}
              <Pressable onPress={replayTarget} style={styles.replayButton}>
                <MaterialIcons name="volume-up" size={36} color="#FFFFFF" />
              </Pressable>
              <Text style={styles.replayHint}>Touche pour écouter</Text>
              ...
              {picked ? (
                ...
                >
                  ...
                  >
                    {isCorrect
                      ? "Bravo ! C'est la bonne lettre !"
                      : `C'était ${current.target}. Essaie encore !`}
                  </Text>
                </Animated.View>
              ) : null}
            </>
...
const styles = StyleSheet.create({
  ...
  progressFill: { height: "100%", backgroundColor: "#FFD966", borderRadius: 7 },
  scoreBadge: {
    backgroundColor: "#FFFFFF",
    borderRadius: 999,
  ...
  instructionTarget: { color: "#FF6B9D", fontSize: 34 },
  ...
  finishTitle: { fontSize: 34, fontWeight: "900", color: "#FF6B9D" },
  finishScore: {
    fontSize: 56,
    fontWeight: "900",
    color: "#FFD966",
