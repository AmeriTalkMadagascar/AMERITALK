...
import { buildPracticeSession, type PracticeExercise, type SessionOptions } from "@/lib/practice-session";
import { pronunciationFeedback, pronunciationScore as calculatePronunciationScore, shouldAutoStopVoice, type PracticeMode } from "@/lib/practice";
import { preloadAudio, speakNatural } from "@/lib/speech";

type SessionMode = "Daily" | PracticeMode;

function modeToOptions(mode: SessionMode, weakIds: string[]): SessionOptions {
  switch (mode) {
    case "Vocabulary":
      return { count: 10, includePronunciation: false };
    case "Listening":
      return { count: 10, includePronunciation: false };
    case "Pronunciation":
      return { count: 6, includePronunciation: true };
    case "Mistakes":
      return { sentenceIds: weakIds.length ? weakIds : undefined, count: 10, includePronunciation: true };
    case "Daily":
    default:
      return { count: 10, includePronunciation: true };
  }
}
...
export default function PracticeSessionScreen() {
  ...
  const mode = (params.mode as SessionMode) || "Daily";
  ...
  const [queue] = useState<PracticeExercise[]>(() => {
    const weakIds = weakWords.length ? weakWords : learnedWords.slice(0, 5);
    return buildPracticeSession(modeToOptions(mode, weakIds));
  });
  ...
  // Préchargement uniquement — AUCUN auto-play
  useEffect(() => {
    preloadAudio(queue.map((ex) => ex.sentence.english));
  }, [queue]);
  ...
  const submit = (value: string, expected: string, sentenceId: string) => {
    ...
    if (right) {
      ...
      recordPractice(100, sentenceId);
    } else {
      loseHeart();
      addMistake(sentenceId);
      recordAnswer(sentenceId, false);
      recordPractice(0, sentenceId);
    }
  };
  ...
  const analyzeRecording = async () => {
    ...
    try {
      ...
      recordPronunciation(score, current.sentence.id);
      markWordLearned(current.sentence.id);
          ...
          {gameOver ? (
            <View>
              ...
              <Pressable onPress={() => router.back()} style={styles.secondary}>
                <Text style={styles.secondaryText}>Retour au hub</Text>
              </Pressable>
            </View>
          ) : finished ? (
            <View>
              <View style={styles.resultCard}>
                ...
                <Text style={styles.resultText}>
                  {correctCount}/{attempts} bonnes réponses.
                </Text>
                <View style={styles.stats}>
                  ...
                  <View style={styles.statItem}>
                    <MaterialIcons name="bolt" size={18} color={duo.gold} />
              ...
              <Pressable onPress={finish} style={styles.primary}>
                <Text style={styles.primaryText}>RETOUR AU HUB</Text>
              </Pressable>
            </View>
          ) : current ? (
            <>
              ...
              {current.kind === "translate" ? (
                <View>
                  ...
                  <View style={styles.bottomArea}>
                    {answer ? <FeedbackBanner correct={!!isCorrect} /> : null}
                    <Pressable
                      disabled={!answer}
                      onPress={goNext}
                ...
                <View>
                  ...
                  <View style={styles.bottomArea}>
                    {pronunciationScore !== null ? (
                      ...
                      <Pressable onPress={skipPronunciation} style={styles.secondary}>
...
// ===== Helpers =====
function getStepTag(ex: PracticeExercise): string {
  switch (ex.kind) {
    case "translate":
...
function getStepTitle(ex: PracticeExercise): string {
  switch (ex.kind) {
    case "translate":
      return "Traduis cette phrase";
    case "build":
...
const styles = StyleSheet.create({
  ...
  buildArea: { minHeight: 80, justifyContent: "center", marginBottom: 30 },
  emptyLines: { gap: 14 },
