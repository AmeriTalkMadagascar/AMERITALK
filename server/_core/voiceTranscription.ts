...
export async function transcribeAudioData(
  ...
  try {
    ...
    const formData = new FormData();
    formData.append("file", new Blob([new Uint8Array(audioBuffer)], { type: mimeType }), `practice.${getFileExtension(mimeType)}`);
    formData.append("model", "whisper-1");
    formData.append("response_format", "verbose_json");
    formData.append("language", language);
    formData.append("prompt", prompt || "Transcribe this English language learning response exactly.");
    const baseUrl = ENV.forgeApiUrl.endsWith("/") ? ENV.forgeApiUrl : `${ENV.forgeApiUrl}/`;
